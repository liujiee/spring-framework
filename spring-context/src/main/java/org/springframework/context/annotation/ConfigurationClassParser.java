...
class ConfigurationClassParser {
	...
	protected void processConfigurationClass(ConfigurationClass configClass, Predicate<String> filter) throws IOException {
		...
		/**
		 * 处理配置类，由于配置类可能存在父类（若父类的全类名是以 java 开头的，则除外），
		 * 		需要將 configClass包装成sourceClass去解析，然后返回其父类。若父类为空则跳出 while 循环
		 * 	SourceClassd的意思：简单的包装类，目的是为了以统一的方式处理带有注解的类，不管这些类是如何加载的
		 */
