...
final class PostProcessorRegistrationDelegate {
	...
	public static void invokeBeanFactoryPostProcessors(
		...
		if (beanFactory instanceof BeanDefinitionRegistry) {
			...
			/**
			 * 到此， 作为 BeanDefinitionRegistryPostProcessor 身份的任务已经结束
		...
		/**
		 * 代码执行到此处，所有的 BeanDefinition 都已经准备完成。 beanFactory#beanDefinitionMap 中有我们用到的所有 BeanDefinition
		 */

		/**
		 * 考虑到上述流程中可能有新的BeanFactoryPostProcessor（BeanDefinitionRegistryPostProcessor只执行一次，
		 * 不然就会无限递归下去） 被注册进来。 Spring 需要重新获取一次，然后分类、排序，找到没有执行过的 BeanFactoryPostProcessor
		 * 执行 postProcessBeanFactory(beanFactory)
		 */
